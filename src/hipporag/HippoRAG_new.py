...
class HippoRAG:
    ...
    def rag_qa(self,
        ...
        Returns:
            Union[
                ...
                - List of raw response messages from the language model.
                - List of metadata dictionaries associated with the results.
    ...
    def graph_search_with_fact_entities(self, query: str,
                                        ...
                                        ppr_method: str = 'standard',
                                        ppr_options: dict = None) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        Parameters:
            ...
            ppr_method (str): The method to use for personalized PageRank computation.
                Options: 'standard', 'montecarlo', 'bidirectional', 'dynamic'
            ppr_options (dict): Additional options for the specific PPR method.
        ...
        # Select PPR method based on the parameter
        if ppr_options is None:
            ppr_options = {}
            
        if ppr_method == 'montecarlo':
            # Use Monte Carlo PPR
            alpha = ppr_options.get('alpha', 0.85)
            num_walks = ppr_options.get('num_walks', 1000)
            walk_length = ppr_options.get('walk_length', 10)
            
            # Convert node weights to a probability distribution
            if sum(node_weights) > 0:
                normalized_weights = node_weights / sum(node_weights)
            else:
                normalized_weights = np.zeros_like(node_weights)
                normalized_weights[0] = 1.0  # Default to first node if all weights are zero
            
            # Find top weighted nodes to use as seeds
            top_node_indices = np.argsort(normalized_weights)[-5:][::-1]  # Top 5 nodes
            
            # Run Monte Carlo PPR for each seed and average the results
            all_scores = {}
            for seed_idx in top_node_indices:
                if normalized_weights[seed_idx] > 0:
                    seed_name = self.graph.vs[seed_idx]['name']
                    scores = self.monte_carlo_ppr(query, alpha=alpha, num_walks=num_walks, walk_length=walk_length, seed_idx=seed_idx)
                    
                    # Accumulate scores
                    for node, score in scores.items():
                        if node in all_scores:
                            all_scores[node] = all_scores[node] + score * normalized_weights[seed_idx]
                        else:
                            all_scores[node] = score * normalized_weights[seed_idx]
            
            # Extract document scores from the passage nodes
            doc_scores = np.zeros(len(self.passage_node_idxs))
            for i, idx in enumerate(self.passage_node_idxs):
                node_name = self.graph.vs[idx]['name']
                if node_name in all_scores:
                    doc_scores[i] = all_scores[node_name]
            
            # Sort and return document IDs and scores
            ppr_sorted_doc_ids = np.argsort(doc_scores)[::-1]
            ppr_sorted_doc_scores = doc_scores[ppr_sorted_doc_ids.tolist()]
            
        elif ppr_method == 'bidirectional':
            # Use Bidirectional PPR (simplified approach for integration)
            alpha = ppr_options.get('alpha', 0.85)
            push_eps = ppr_options.get('push_eps', 1e-4)
            num_walks = ppr_options.get('num_walks', 1000)
            
            # Fallback to standard PPR for now with a warning
            logger.warning(f"Bidirectional PPR not fully integrated with graph_search_with_fact_entities yet. Using standard PPR.")
            ppr_sorted_doc_ids, ppr_sorted_doc_scores = self.run_ppr(node_weights, damping=self.global_config.damping)
            
        elif ppr_method == 'dynamic':
            # Use Dynamic Teleport PPR
            alpha = ppr_options.get('alpha', 0.85)
            iters = ppr_options.get('iters', 20)
            top_m = ppr_options.get('top_m', 5)
            
            # Get dynamic teleport vector based on query and entity scores
            scores = self.dynamic_teleport_ppr(query, alpha=alpha, iters=iters, top_m=top_m)
    ...
    def monte_carlo_ppr(self, query, alpha=0.85, num_walks=1000, walk_length=10, seed_idx=None):
        """
        ...
        Parameters:
            query (str): The query string used to find the seed node.
            alpha (float): Probability of following graph edges vs teleporting back.
            num_walks (int): Number of random walks to perform.
            walk_length (int): Maximum length of each random walk.
            seed_idx (int, optional): Index of the seed node in the graph.
                                     If None, a seed will be determined from the query.
        
        Returns:
            Dict[str, float]: Node names mapped to their PPR scores.
        """
        logger.info(f"[MonteCarloPPR] query={query}, walks={num_walks}, length={walk_length}")
        
        # Use provided seed_idx or find seed nodes by query similarity
        if seed_idx is None:
            # Find seed nodes by query similarity - use linking logic
            query_embedding = self.embedding_model.embed_query(query)
            entity_scores = self.entity_embedding_store.search(query_embedding, top_k=5)
            
            # If no entities found, return empty results
            if not entity_scores:
                logger.warning(f"No seed entities found for query: {query}")
                return {n: 0.0 for n in self.graph.vs['name']}
                
            # Use top entity as seed
            seed_name = entity_scores[0].value
            seed_idx = self.node_name_to_vertex_idx.get(seed_name)
            
            if seed_idx is None:
                logger.warning(f"Seed {seed_name} not found in graph; returning zero vector.")
                return {n: 0.0 for n in self.graph.vs['name']}
        
        # Initialize visit counts
        counts = {idx: 0 for idx in range(len(self.graph.vs))}
        for _ in range(num_walks):
            current = seed_idx
            counts[current] += 1
            for _ in range(walk_length):
                nbrs = self.graph.neighbors(current, mode='out')
        ...
        scores = {self.graph.vs[n]['name']: counts[n] / total for n in counts}
        
        return scores
    
    def bidirectional_ppr(self, source_query, target_query, alpha=0.85, push_eps=1e-4, num_walks=1000):
        """
        ...
        Parameters:
            source_query (str): Query to find source node.
            target_query (str): Query to find target node.
            alpha (float): Damping factor for PageRank.
            push_eps (float): Threshold for residual propagation.
            num_walks (int): Number of walks for Monte Carlo phase.
            
        Returns:
            float: Approximate PPR score from source to target.
        """
        logger.info(f"[BidirectionalPPR] source={source_query}, target={target_query}, walks={num_walks}")
        
        # Find source and target nodes
        source_emb = self.embedding_model.embed_query(source_query)
        target_emb = self.embedding_model.embed_query(target_query)
        
        source_matches = self.entity_embedding_store.search(source_emb, top_k=1)
        target_matches = self.entity_embedding_store.search(target_emb, top_k=1)
        
        if not source_matches or not target_matches:
            logger.warning("Source or target not found; returning zero.")
            return 0.0
            
        source_name = source_matches[0].value
        target_name = target_matches[0].value
        
        source_idx = self.node_name_to_vertex_idx.get(source_name)
        target_idx = self.node_name_to_vertex_idx.get(target_name)
        
        if source_idx is None or target_idx is None:
            logger.warning(f"Source {source_name} or target {target_name} not in graph; returning zero.")
            return 0.0
        
        # Initialize residual and p-vectors
        nodes = list(range(len(self.graph.vs)))
        residual = {n: 0.0 for n in nodes}
        p = {n: 0.0 for n in nodes}
        
        residual[target_idx] = 1.0
        queue = [target_idx]
        ...
        while queue:
            ...
            for u in self.graph.neighbors(v, mode='in'):
                deg = self.graph.degree(u, mode='out')
        ...
        for _ in range(num_walks):
            u = source_idx
            while True:
                nbrs = self.graph.neighbors(u, mode='out')
                if random.random() < alpha and nbrs:
    ...
    def iterative_rag(self, query, hops=3, top_k=10):
        """
        ...
        Parameters:
            ...
            hops (int): Number of expansion iterations.
            top_k (int): Number of documents to retrieve per iteration.
            
        Returns:
            List[str]: Document IDs after iterative retrieval.
        """
        logger.info(f"[IterativeRAG] query='{query}', hops={hops}")
        
        # Initial retrieval
        query_embedding = self.embedding_model.embed_query(query)
        docs = self.chunk_embedding_store.search(query_embedding, top_k=top_k)
        doc_ids = [doc.id for doc in docs]
        
        for i in range(hops):
            # Extract entities from current documents
            entities = set()
            doc_contents = [self.chunk_embedding_store.get_string(doc_id) for doc_id in doc_ids]
            
            # Use the LLM to extract entities
            prompt = f"Extract important entities from the following text:\n\n{' '.join(doc_contents)}\n\nEntities (one per line):"
            try:
                entity_text = self.llm_model.generate(prompt)
                extracted_entities = [e.strip() for e in entity_text.split('\n') if e.strip()]
                entities.update(extracted_entities)
            except Exception as e:
                logger.error(f"Error extracting entities: {str(e)}")
                
            # Find entity matches in the graph
            entity_nodes = []
            for entity in entities:
                for v in self.graph.vs:
                    if entity.lower() in v['name'].lower():
                        entity_nodes.append(v.index)
            
            # Get neighbors of found entities
            neighbors = set()
            for e_idx in entity_nodes:
                nbrs = self.graph.neighbors(e_idx)
                neighbors.update(nbrs)
            
            # Skip if no neighbors found
            if not neighbors:
                logger.warning(f"No neighbors found at hop {i+1}; using previous docs")
                continue
                
            # Create expanded query with neighbor information
            ...
            new_docs = self.chunk_embedding_store.search(
                self.embedding_model.embed_query(expanded_query), 
                top_k=top_k
            )
    ...
    def dynamic_teleport_ppr(self, query, alpha=0.85, iters=20, top_m=5):
        """
        Dynamic Teleportation PPR:
        1. Compute cosine similarity with node embeddings to select top_m seeds.
        2. Build teleport vector and run power iteration for `iters`.
        
        Parameters:
            query (str): Query string.
            alpha (float): Damping factor for PageRank.
            iters (int): Number of power iterations.
            top_m (int): Number of seed nodes to use.
            
        Returns:
            Dict[str, float]: PPR scores for each node.
        """
        logger.info(f"[DynamicTeleportPPR] query='{query}', iters={iters}, top_m={top_m}")
        
        # Obtain query embedding
        q_emb = self.embedding_model.embed_query(query)
        
        # Get entity embeddings
        entity_scores = self.entity_embedding_store.search(q_emb, top_k=top_m * 2)
        
        if not entity_scores:
            logger.warning("No matching entities; returning zero vector.")
            return {self.graph.vs[n]['name']: 0.0 for n in range(len(self.graph.vs))}
        
        # Build teleport vector using selected entities
        seeds = []
        scores = {}
        for score_obj in entity_scores:
            entity_name = score_obj.value
            if entity_name in self.node_name_to_vertex_idx:
                idx = self.node_name_to_vertex_idx[entity_name]
                seeds.append(idx)
                scores[idx] = score_obj.score
        
        # Take only top_m seeds
        seeds = seeds[:top_m]
        
        if not seeds:
            logger.warning("No valid seeds found; returning zero vector.")
            return {self.graph.vs[n]['name']: 0.0 for n in range(len(self.graph.vs))}
            
        # Create teleport vector
        teleport = {n: (scores[n] if n in seeds else 0.0) for n in range(len(self.graph.vs))}
        ...
        for _ in range(iters):
            ...
            for u in range(len(self.graph.vs)):
                out_edges = self.graph.incident(u, mode='out')
                if not out_edges:
                    continue
                
                neighbors = self.graph.neighbors(u, mode='out')
                deg = len(neighbors)
                if deg == 0:
                    continue
                    
                for v in neighbors:
                    new_p[v] = new_p.get(v, 0.0) + alpha * p[u] / deg
                    
            p = new_p
        
        # Convert indices back to node names
        return {self.graph.vs[n]['name']: p[n] for n in p}
    
    def graph_search_rpep(self, query, query_embedding=None):
        """
        Relation Prune + Entity Prune search:
        1. Seed from top entities.
        2. For max_depth=2:
        ...
        Returns:
            Tuple[List[str], List[float]]: Tuple of document IDs and scores.
        """
        logger.info(f"[RP+EP] query='{query}'")
        
        if query_embedding is None:
            query_embedding = self.embedding_model.embed_query(query)
        
        # Initialize seeds from entity embeddings
        entity_matches = self.entity_embedding_store.search(
            query_embedding, 
            k=self.global_config.linking_top_k
        )
        
        current = set()
        for match in entity_matches:
            entity_name = match.value
            if entity_name in self.node_name_to_vertex_idx:
                current.add(self.node_name_to_vertex_idx[entity_name])
        ...
        for _ in range(2):
            ...
            for ent_idx in current:
                ent_name = self.graph.vs[ent_idx]['name']
                
                # Get outgoing edges and their relations
                out_edges = self.graph.incident(ent_idx, mode='out')
                rels = set()
                for e_idx in out_edges:
                    edge = self.graph.es[e_idx]
                    rel = edge['relation'] if 'relation' in edge.attributes() else "connected"
                    if rel.lower() != 'synonym':
                        rels.add(rel)
                
                if not rels:
                    continue
                
                # Prepare prompt for relation scoring
                prompt = f"Entity: {ent_name}\nQuestion: {query}\nRelations:\n"
                prompt += '\n'.join(f"- {r}" for r in rels)
                prompt += "\nRate each relation from 0-10 based on relevance to the question. List only the top relations with scores >= 2."
                ...
                try:
                    ...
                    # Get top relations with scores >= 2
                    top_rels = [r for r, sc in sorted(scores_map.items(), key=lambda x: -x[1]) if sc >= 2][:5]
                    
                    # Collect next-hop entities through top relations
                    for e_idx in out_edges:
                        edge = self.graph.es[e_idx]
                        rel = edge['relation'] if 'relation' in edge.attributes() else "connected"
                        if rel in top_rels:
                    ...
                    logger.error(f"Error in relation pruning: {str(e)}")
            
            # Entity prune via chunk embeddings
            ent_scores = []
            for ent_idx in next_ent:
                name = self.graph.vs[ent_idx]['name']
                try:
                    combined_query = f"{name} {query}"
                    results = self.chunk_embedding_store.search(
                        self.embedding_model.embed_query(combined_query), 
                        top_k=10
                    )
                    score = results[0].score if results else 0
                    ent_scores.append((ent_idx, score, results))
                except Exception as e:
                    logger.error(f"Error in entity pruning for {name}: {str(e)}")
        ...
        final = []
        for _, _, results in pruned:
            if results:
                final.append((results[0].id, results[0].score))
        
        final.sort(key=lambda x: -x[1])
        if not final:
            return [], []
            
        ids, scores = zip(*final)
    ...
    def compute_text_similarity(self, text1, text2):
        ...
        Returns:
            float: Jaccard similarity score between the two texts
        """
        ...
        return len(intersection) / (len(tokens1) + len(tokens2) - len(intersection))  # Jaccard similarity